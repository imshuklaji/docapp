...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"docchannel\")"
  ...
  echo
  echo "	fabricNetwork.sh generate -c docchannel"
  echo "	fabricNetwork up -c docchannel -s couchdb"
  echo "        fabricNetwork up -c docchannel -s couchdb -i 1.4.0"
  echo "	fabricNetwork up -l node"
  echo "	fabricNetwork down -c docchannel"
  echo
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    docker run -v "$PWD":/tmp/docchannel --rm hyperledger/fabric-tools:"$IMAGETAG" rm -Rf /tmp/docchannel/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/allparticipants.doc-app.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID doc-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile docchannel -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile docchannel -outputAnchorPeersUpdate ./channel-artifacts/allparticipantsMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg allparticipantsMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
...
CLI_DELAY=5
# channel name defaults to "docchannel"
CHANNEL_NAME="docchannel"
